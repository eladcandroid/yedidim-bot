...
exports.sendFollowerNotification = functions.database.ref('/events/{eventId}').onCreate(event => {
  if (!isProduction) {
    sendFollowerNotification.handleUpdateEvent(event, admin);
  }
});

exports.sendExpoFollowerNotification = functions.database.ref('/events/{eventId}').onCreate(event => {
  if (!isProduction) {
