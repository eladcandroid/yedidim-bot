...
exports.sendNotificationBySearchRadius = (req, res, admin) => {
  return new Promise((resolve, reject) => {
    ...
    admin.database().ref('/events/' + eventId).once('value', snapshot => {
      let event = snapshot.val();
      if (event) {
        sendNotificationToCloseByVolunteers(admin, event, 'קריאה חדשה', searchRadius).then(() => {
          res.status(200).send('');
          resolve();
        }).catch(err => {
      ...
      if (event) {
        admin.database().ref('/volunteer/' + recipient).once('value', snapshot => {
          ...
          if (user) {
            ...
            sendNotifications([notification]).then(recipients => {
...
const normalizePhone = phone => phone.charAt(0) === '0' ? '+972' + phone.substr(1) : phone;

// if dispatcher then "/dispatchers"
// otherwise admin or volunteer - "/volunteer"
const userRoleToPath = (role, userId) => `/${role === 'dispatcher' ? 'dispatchers' : 'volunteer'}/${userId}`;
...
exports.sendTestNotification = (() => {
  var _ref = (0, _asyncToGenerator3.default)(function* (req, res, admin) {
    ...
    if (!userId || !role) {
      console.log(`[SendTestNotification]A: Sending test notification to all users`, userId, role);
      const volunteers = yield admin.database().ref('/volunteer').once('value');

      volunteers.forEach(function (entry) {
        users.push(userToJSON(entry.key, entry.val(), 'volunteer'));
      });

      const dispatchers = yield admin.database().ref('/dispatchers').once('value');

      dispatchers.forEach(function (entry) {
        users.push(userToJSON(entry.key, entry.val(), 'dispatcher'));
      });
    } else {
      console.log(`[SendTestNotification]A: Sending test notification to specific user`, userId, role);
      // Grab only one user
      const user = yield admin.database().ref(userRoleToPath(role, userId)).once('value');
    ...
    yield firebase.database().ref().update(users.reduce(function (acc, user) {
    ...
    const receipts = yield sendNotifications(users.map(function (user) {
      return {
        to: user.token,
    ...
    console.log(`[SendTestNotification]D: Writing new status based on receipts`, receipts);
    // TODO
  });
...
let sendNotificationToCloseByVolunteers = (admin, eventData, notificationTitle, searchRadius) => {
  return new Promise((resolve, reject) => {
    ...
    geoQuery.on('ready', function () {
      ...
      admin.database().ref('/volunteer').orderByChild('NotificationToken').startAt('').once('value', tokens => {
        ...
        let recipients = Object.keys(usersById).filter(userId => Expo.isExpoPushToken(usersById[userId].NotificationToken) && usersInRadius.indexOf(userId) > -1 && !userMutedNotifications(usersById[userId]));
        ...
        sendNotifications(notifications).then(() => resolve(recipients)).catch(err => {
          reject(err);
        });
      });
    });
